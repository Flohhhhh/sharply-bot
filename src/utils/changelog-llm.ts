...
export async function generateChangelogWithLlm(
	...
	const system = [
		...
		'Your job is to infer themes and write a clear digest that feels like a human PM wrote it. Prefer accuracy and separation over cramming unrelated work together.',
	...
	const user = [
		...
		'- Default: **one bullet per PR** (one id per line) with its own rich sentence—this is the safe choice when titles describe different surfaces, features, or outcomes.',
		'- Merge **only** when two or more items are **obviously** the same user-facing story from the titles alone (e.g. multiple PRs that are clearly all refinements to the same flow, like several tightly related search UX tweaks). If you have to stretch to connect them, **do not** merge.',
		'- Do **not** merge just because items share a vague bucket (e.g. both say "browse" or "mobile") if they fix or ship **different** behaviors.',
		'- When you do merge, write one flowing sentence with clauses (commas / "and" / "plus") and put **every** merged id\'s link token at the end of that bullet—still one bullet, multiple [[id]](<url>) tokens.',
		...
		'- Fixes: under ## Fixes, default to **one bullet per fix PR**; merge only when titles show the **same** bug or the **same** small surface (e.g. two PRs both clearly about the same input field). User-benefit language, not "fixed bug in component".',
		'',
